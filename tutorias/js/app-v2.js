// tutorias/js/app-v2.js
// VERSIÓN DE PRUEBA PARA ACCESO INDIVIDUAL DE MAESTROS
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (sessionName) {
        document.getElementById('user-name').textContent = sessionName;
        document.getElementById('report-teacher-name').textContent = sessionName.toUpperCase();
        
        // Iniciales
        const initials = sessionName.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase();
    ...
    window.toggleTutoriaSexo = async (event, fecha, alumno, sexoActual) => {
        ...
        try {
            const result = await api.actualizarCampo(fecha, alumno, 5, nuevoSexo);
            if (result.status === 'success') {
    ...
    window.toggleTutoriaParcial = async (event, fecha, alumno, parcialActual) => {
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                alert("Error al actualizar: " + result.message);
                badge.innerHTML = "P" + parcialActual;
                badge.style.opacity = "1";
                badge.style.pointerEvents = "auto";
            }
        } catch (error) {
    ...
    function setupGroupSelection() {
        ...
        btnAddManualDirect.addEventListener('click', () => {
            ...
            if (name.length < 5) { alert("Por favor escribe el nombre completo."); return; }
            const sex = manualSexSelect.value;
            const newStudent = { nombre: name, sexo: sex, grupo: inputGrupo.value.trim().toUpperCase() || "", isSelected: true };
            if (!selectedStudentsState.find(s => s.nombre === name)) { selectedStudentsState.unshift(newStudent); }
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (finalSelection.length === 0) { alert("Por favor, selecciona o añade al menos un estudiante."); return; }
