// tutorias/js/app-asistencia.js (VERSIÓN DE PRUEBA CON ASISTENCIA)
...
document.addEventListener('DOMContentLoaded', async () => {
        ...
        try {
            ...
            if (userEmail) {
                ...
                allData.tutorias = (allData.tutorias || []).filter(t => 
                    String(t.docenteEmail || t.docente_email || "").toLowerCase().trim() === userEmail.toLowerCase().trim()
                );
    ...
    function renderDashboard() {
        ...
        if (recent.length > 0) {
            list.innerHTML = recent.map(t => {
                const attended = String(t.asistencia || "").toUpperCase().startsWith('S') || t.asistencia === true;
                return `
                    <div style="display:flex; justify-content:space-between; align-items:center; padding:12px; background:#f8fafc; border-radius:10px; border:1px solid #f1f5f9;">
                        <div style="display:flex; align-items:center; gap:10px;">
                            <div title="${attended ? 'Asistió' : 'No asistió'}" style="color: ${attended ? '#10b981' : '#ef4444'}; font-size: 1.2rem;">
                                ${attended ? '✅' : '❌'}
                            </div>
                            <div>
            ...
            return `
                <tr style="border-bottom: 1px solid #f8fafc;">
                    ...
                    <td style="padding:1rem; text-align:center;">
                        <span onclick="toggleTutoriaAsistencia(event, '${t.fecha}', '${t.alumno.replace(/'/g, "\\'")}', '${t.asistencia}')"
                              title="Click para cambiar asistencia"
                              style="cursor:pointer; font-size:1.1rem; filter: grayscale(${attended ? 0 : 1}); opacity: ${attended ? 1 : 0.5};">
                            ${attended ? '✅' : '❌'}
                        </span>
                    </td>
                    ...
                    <td style="padding:1rem; font-size:0.85rem; color:#64748b; max-width:200px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${t.tema || t.motivo || ""}</td>
                    <td style="padding:1rem; font-size:0.85rem;">
    ...
    window.toggleTutoriaAsistencia = async (event, fecha, alumno, actual) => {
        const attended = String(actual || "").toUpperCase().startsWith('S') || actual === true;
        const nuevoValor = attended ? "NO" : "SÍ";
    ...
    window.toggleTutoriaSexo = async (event, fecha, alumno, sexoActual) => {
        ...
        try {
            const result = await api.actualizarCampo(fecha, alumno, "sexo", nuevoSexo); 
            if (result.status === 'success') {
                await loadInitialData(); 
            } else {
                alert("Error al actualizar: " + result.message);
    ...
    window.toggleTutoriaParcial = async (event, fecha, alumno, parcialActual) => {
        ...
        try {
            const result = await api.actualizarCampo(fecha, alumno, 2, nuevoParcial); 
            if (result.status === 'success') {
                await loadInitialData(); 
            } else {
                alert("Error al actualizar: " + result.message);
        ...
        repTbody.innerHTML = filtered.map(t => {
            ...
            return `
                <tr>
                    ...
                    <td style="border:1px solid #000; padding:4px; text-align:center; font-weight:bold;">${attended ? 'S' : 'N'}</td>
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const attends = formData.get('asistencia') === 'on';

        const payload = {
            parcial: formData.get('parcial'),
            semestre: "FEBRERO-JULIO 2026", // Por defecto según el reporte
            ...
            motivo: formData.get('tema'), // Mapeado a 'motivo' en el backend v3.2
            acuerdos: "", // Campo nuevo en v3.2
            tipo: formData.get('tutoria_tipo') === 'individual' ? 'Individual' : 'Grupal',
            asistencia: attends ? "SÍ" : "NO"
        };
        ...
        try {
            ...
            const btn = form.querySelector('button[type="submit"]');
            btn.disabled = false;
            btn.textContent = "Registrar Sesión (TEST V3)";
        }
    });
