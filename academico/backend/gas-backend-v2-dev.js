...
function importarDesdeParser(data) {
  ...
  if (shCals.getLastRow() === 0) {
    shCals.appendRow(["id","curp","nombre","grupo","semestre","turno","ciclo",
                      "parcial","materia","docente","calificacion","faltas","sinDatos",
  ...
  // --- AUTO-CORRECTOR DE COLUMNAS ---
  // Nos aseguramos que la columna 'docente' exista para evitar desalineación
  let currentHeaders = shCals.getRange(1, 1, 1, Math.max(1, shCals.getLastColumn())).getValues()[0].map(h => String(h).toLowerCase().trim());
  if (currentHeaders.indexOf("docente") === -1) {
    let idxMateria = currentHeaders.indexOf("materia");
    if (idxMateria !== -1) {
      shCals.insertColumnAfter(idxMateria + 1);
      shCals.getRange(1, idxMateria + 2).setValue("docente");
    }
  }
  // ----------------------------------
  ...
  data.alumnos.forEach(al => {
    ...
    al.calificaciones.forEach(c => {
      ...
      // --- NORMALIZACIÓN Y ASIGNACIÓN DE DOCENTE ---
      const infoOficial = buscarEnDirectorioSync(dir, c.materia, al.grupo);
      const materiaFinal = infoOficial.materia || c.materia;
      const docenteFinal = infoOficial.docente || "—";

      idBase++;
      rowsCals.push([
        idBase,
        al.curp, al.nombre, al.grupo, al.semestre, al.turno, al.ciclo,
        al.parcial, materiaFinal, docenteFinal,
        c.cal !== null ? c.cal : "",      // conservar decimales (ej: 8.5)
        Math.round(c.faltas || 0),       // faltas siempre entero
        c.sinDatos ? "SI" : "NO",
        al.promedio || "",
        al.enRiesgo ? "SI" : "NO",
        fuente, timestamp
...
function limpiarParcial(grupo, parcial, ciclo) {
  ...
  if (!sh || sh.getLastRow() <= 1) return { status: "success", borradas: 0 };
  ...
  // Filtramos: nos quedamos con todo lo que NO coincida con lo que queremos borrar
  const filteredData = fullData.filter((row, idx) => {
    if (idx === 0) return true; // Encabezado siempre se queda
    const matchParcial = String(row[iParcial]) === String(parcial);
    const matchCiclo   = String(row[iCiclo])   === String(ciclo);
    const matchGrupo   = !grupo || grupo === "TODOS" || String(row[iGrupo]) === String(grupo);
    return !(matchParcial && matchCiclo && matchGrupo);
  });

  const borradas = fullData.length - filteredData.length;
  if (borradas > 0) {
    sh.clearContents();
    sh.getRange(1, 1, filteredData.length, filteredData[0].length).setValues(filteredData);
  }

  // --- Opcional: Limpiar Acad_Alumnos si es TODOS ---
  if (!grupo || grupo === "TODOS") {
    ...
    if (shA && shA.getLastRow() > 1) {
      ...
      const filteredA = dataA.filter((row, idx) => {
        if (idx === 0) return true;
        return String(row[iCicloA]) !== String(ciclo);
      });
      if (dataA.length !== filteredA.length) {
        shA.clearContents();
        shA.getRange(1, 1, filteredA.length, filteredA[0].length).setValues(filteredA);
      }
    }
  }
...
function calcPorMateria(cals) {
  ...
  cals.forEach(c => {
    ...
    if (!grupos[key].materias[c.materia]) {
      grupos[key].materias[c.materia] = { califs: [], grupo: c.grupo, docente: c.docente };
    }
  ...
  Object.values(grupos).sort((a,b) => {
    ...
    Object.entries(g.materias).forEach(([materia, d]) => {
      // Priorizar docente ya guardado en la data de calificaciones (si existe en los registros crudos)
      // Pero calcPorMateria a veces recibe cals ya procesadas que no tienen docente al nivel de materia/grupo.
      // Aquí intentaremos buscarlo en el registro o volver al directorio.
      const primerCal = d.primerCalRecord; // Necesitamos guardar esto en el bucle anterior
      const doc = (d.docente && d.docente !== "—") ? d.docente : buscarDocente(dir, materia, d.grupo || "");
      filas.push({
        semestre:    g.semestre,
        turno:       g.turno,
        materia,
...
/**
 * Helper para buscar el docente (usado en reportes)
 */
function buscarDocente(dir, materia, grupo) {
  const res = buscarEnDirectorioSync(dir, materia, grupo);
  return res.docente || "—";
}

/**
 * Busca info oficial de materia y docente en un directorio ya cargado
 */
function buscarEnDirectorioSync(dir, materiaRaw, grupo) {
  const nm = norm(materiaRaw.split(" (Sem")[0]); // Quitar el sufijo de semestre para buscar
  const ng = norm(grupo);

  // Intentar match por materia y grupo
  for (const row of dir.rows) {
    const mOficial = String(row[1] || "");
    const gOficial = String(row[0] || "");
    const nmOficial = norm(mOficial);
    const ngOficial = norm(gOficial);
    
    if (ngOficial === ng) {
      // A. Coincidencia Total o Parcial (rápida)
      if (nmOficial === nm || nmOficial.includes(nm) || nm.includes(nmOficial)) {
        return { materia: mOficial, docente: String(row[2] || "") };
      }
      
      // B. Coincidencia por Palabras Clave (Tokens)
      const tokensPDF = nm.split(/[\s\.]+/).filter(t => t.length >= 2);
      const tokensOfi = nmOficial.split(/[\s\.]+/).filter(t => t.length >= 2);
      
      // Si el PDF tiene pocas palabras (ej: "Ingles II"), buscamos si todas están en el Oficial
      const coincidencias = tokensPDF.filter(t => tokensOfi.some(to => to.includes(t) || t.includes(to))).length;
      const score = coincidencias / tokensPDF.length;
  ...
  // Fallback: búsqueda global si no hubo match con grupo (mismo algoritmo)
  for (const row of dir.rows) {
    const mOficial = String(row[1] || "");
    const nmOficial = norm(mOficial);
    const tokensPDF = nm.split(/[\s\.]+/).filter(t => t.length >= 2);
    const tokensOfi = nmOficial.split(/[\s\.]+/).filter(t => t.length >= 2);
    const coincidencias = tokensPDF.filter(t => tokensOfi.some(to => to.includes(t) || t.includes(to))).length;
    if (tokensPDF.length > 0 && (coincidencias / tokensPDF.length) >= 0.8) {
      return { materia: mOficial, docente: String(row[2] || "") };
    }
  }

  return { materia: null, docente: null };
}
...
function calcPorDocente(cals) {
  ...
  cals.filter(c => c.calificacion !== null).forEach(c => {
    const doc = c.docente && c.docente !== "—" ? c.docente : buscarDocente(dir, c.materia, c.grupo);
