...
function renderSummaries() {
  ...
  if (!root) return;
  const all = storage.getAllSummaries();
    ...
    <details class="add-summary-picker" style="margin-bottom:0.85rem">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:0.4rem;margin-top:0.7rem">
        ${CATALOG.documents.map(d => {
          const has = storage.hasSummary(d.id);
...
function renderStats() {
  ...
  const totalSummaries = storage.getAllSummaries().length;
  $("#stats").innerHTML = `
    <div class="stat-block"><strong>${totalSummaries}</strong><span>Resúmenes</span></div>
