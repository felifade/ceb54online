...
export function openSummaryEditor({ doc, onClose }) {
  ...
  const initialText = storage.getSummary(doc.id);
  let currentText = initialText;
  let saveTimer = null;
  const persist = (immediate = false) => {
    clearTimeout(saveTimer);
    if (immediate) { storage.setSummary(doc.id, currentText); return; }
    saveTimer = setTimeout(() => storage.setSummary(doc.id, currentText), 700);
  };
