...
function doGet(e) {
  try {
    ...
    // 9. RETROALIMENTACIÓN DOCENTE (HISTORIAL COMPLETO PARA ANALÍTICAS)
    let feedbackHistory = [];
    const sFHistory = getSheet(ss, S_FEEDBACK);
    if (sFHistory) {
      const dF = sFHistory.getDataRange().getValues(); dF.shift();
...
function doPost(e) {
  try {
    ...
    if (action === "updateTutoriaField") {
       ...
       if (!matched) return ContentService.createTextOutput(JSON.stringify({ status: "error", message: `No se encontró la fila. body.col: ${body.column}, tA: ${tA}, colIndex: ${col}` }));
       return ContentService.createTextOutput(JSON.stringify({ status: "success" }));
    }
